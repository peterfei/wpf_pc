...
export default class PartDesectScreen extends Component {
  componentDidMount() {
  ...
  async componentWillMount() {
    this.setState({
      datas: api.part_desect_menus,
      contentViews: api.content_part_desect,
      type_id: 1
    });
  }
  constructor(props) {
  ...
  render() {
    ...
    const contentViews = this.state.contentViews
      .filter(x => x.type_id == this.state.type_id)
      .map((content, i) => {
        return (
          <View
            style={{
    ...
    const menuItems = this.state.datas.map((info, i) => {
      return (
        //onPress={() =>NativeModules.MyDialogModel.SendMessageToUnity(info.ImgName)}
        <TouchableOpacity
          style={{ flex: 1, justifyContent: "center", alignItems: "center" }}
          onPress={() => this.changeContentType(info.TypeId)}>
          <Image
            resizeMode="contain"
            style={styles.icon}
            source={info.ImgName}
          />
          <Button title={info.TypeName} style={{ color: "blue" }} />
        </TouchableOpacity>
      );
    });
    return (
      <View>
        <View
          style={{ flexDirection: "row", flexWrap: "wrap", paddingTop: 20 }}>
          {menuItems}
        </View>
        <SpacingView />
