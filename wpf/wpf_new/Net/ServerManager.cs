...
namespace Vesal_PC.Net
{
    public class ServerManager
    {
        ...
        public static void sendToJs(string data)
        {
            ...
            Context.GetJavaScriptModule<RCTDeviceEventEmitter>().emit("testBind", "message:" + data);
        }
            ...
            while (true)
            {
                ...
                try
                {
                    ...
                    if (myClientSocket.Available != 0)
                    {
                        ...
                        //發送給Js
                        //ServerManager.sendToJs(message);
                        switch (message)
                        {
                            case "disconnect":
...
class vesal_log
{
    static string logPath = AppDomain.CurrentDomain.BaseDirectory + "win/vesal_Data/StreamingAssets/temp/";

    public static String _log_path = logPath + "vesal.log";
